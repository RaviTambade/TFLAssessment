...
public class AssessmentUpcomingRepository : IAssessmentUpcomingRepository
{
    ...
    [HttpGet]
    public async Task<List<AssessmentUpcomingDto>> GetAllUpcomingAssessments(long studentId)
    {
        var assessments = await (
            from a in _context.Assessments
            join t in _context.Tests   on a.TestId equals t.Id into at from t in at.DefaultIfEmpty()
            ...
            where a.StudentId == studentId  && (a.Status.ToString() == "Assigned")
