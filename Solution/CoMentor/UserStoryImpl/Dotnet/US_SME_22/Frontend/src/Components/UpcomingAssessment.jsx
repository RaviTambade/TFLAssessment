...
const UpcomingAssessment = () => {
    ...
    const fetchAssessments = async () => {
      ...
      try {
        const response = await fetch(
          `http://localhost:5201/api/AssessmentUpcoming/${userId}`
        );
  ...
  return (
    <div style={{ padding: "20px", backgroundColor: "#f6e3df", minHeight: "100vh" }}>
      {/* Title */}
      <h2 style={{ color: "#0f172a", fontSize: "28px", fontWeight: "bold" }}>
      ...
      {/* Loading */}
      {loading && <p style={{ marginTop: "15px", color: "#475569" }}>Loading...</p>}

      {/* Error */}
      {error && <p style={{ marginTop: "15px", color: "#ef4444" }}>{error}</p>}
