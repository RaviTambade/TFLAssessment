...
const MentorReviewQuestion = () => {
  ...
  const fetchDraftQuestions = async () => {
    try {
      const res = await fetch(`${BASE_URL}/draft-list`);
  ...
  const fetchRecent = async () => {
    try {
      const res = await fetch(`${BASE_URL}/recent-list`);
      const data = await res.json();
      setQuestions(data);
    } catch (err) {
      console.log(err);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-500 via-indigo-500 to-purple-600 p-6">
      ...
      <h1 className="text-2xl font-bold text-black mb-6 text-center ">Draft Question</h1>
