...
$(document).ready(function () {
  ...
  // selected concept and difficulty level

  $.get(getConceptsUrl, function (concepts) {
    $("#conceptSelect").html(
  ...
  // View Selected Questions (Using Concept ID)
  $("#viewSelectedQuestions").click(function () {

    const conceptTitle = $("#conceptSelect").val();
    const difficultyLevel = $("#difficultySelect").val();

    if (!conceptTitle || !difficultyLevel) {
      alert("Please select concept and difficulty");
      return;
    }

    $.ajax({
      url: `http://localhost:5238/api/questionbank?conceptTitle=${conceptTitle}&difficultyLevel=${difficultyLevel}`,
      method: "GET",
