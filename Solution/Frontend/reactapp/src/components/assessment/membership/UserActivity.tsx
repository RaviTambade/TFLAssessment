...
const UserActivity = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
              const [countRes, loginsRes, avgRes] = await Promise.all([fetch(`${WEBAPI_NODE_URL}/useractivity/active-count`), fetch(`${WEBAPI_NODE_URL}/useractivity/logins-24h`), fetch(`${WEBAPI_NODE_URL}/useractivity/average-time`),]);
    ...
    <div className="min-h-screen bg-gradient-hero flex flex-col">
      <div className="flex-1 p-6 pt-24">
        ...
        {showActiveUsers && (
          <Card className="mb-8 border-l-4 border-red-500 shadow-lg">
            <CardContent className="p-6">
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {activeUsersList.map((u, i) => (
                    <tr key={`${u.full_name}-${u.login_time}-${i}`} className="border-b">
                      <td>{u.full_name}</td>
              ...
              <button onClick={() => setShowActiveUsers(false)} className="mt-4 bg-red-500 text-white px-4 py-2 rounded">
                Close
              </button>
            </CardContent>
          </Card>
        ...
        <div className="flex gap-4 mb-6">
          <input type="text" placeholder="Search sessions by name..." value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}className="flex-1 border px-4 py-2 rounded"/>
          <button onClick={() => setSearchTerm("")} className="bg-primary text-white px-6 py-2 rounded">
            Clear
          </button>
        </div>
        {/* Sessions Table */}
        <Card>
          <CardContent className="p-6">
            {error ? (<div className="text-red-500 text-center py-8"> {error}</div>) : loading ? 
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {filteredSessions.slice(0, 15).map((s, index) => (
                    <tr key={`${s.id}-${index}`} className="border-b h-12">
                      <td>{s.name}</td>
                      <td>{s.login || "-"}</td>
                      <td>{s.logout || "-"}</td>
                      <td>{s.status}</td>
