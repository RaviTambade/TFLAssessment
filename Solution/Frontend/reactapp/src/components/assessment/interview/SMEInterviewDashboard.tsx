...
import {CalendarDays, Monitor, CheckCircle, Clock, Ban, Briefcase, User,} from "lucide-react";
    ...
    <section className="py-16 sm:py-20 bg-background min-h-screen">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-14">
          <h2 className="text-3xl sm:text-4xl font-bold text-foreground mb-4"> Interview{" "}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 mb-14">
          <button onClick={() => { setSelectedType("TOTAL"); setSelectedInterview(null);}}
            ...
            <div className="flex justify-between items-start">
              <div>
                <p className="text-lg font-semibold opacity-90">Total Interviews</p>
                <h2 className="text-4xl font-bold mt-4">{totalInterviews}</h2>
              </div>
          ...
          {/* UPCOMING */}
          <button onClick={() => {setSelectedType("UPCOMING"); setSelectedInterview(null);}}
            ...
            <div className="flex justify-between items-start">
              <div>
                <p className="text-lg font-semibold opacity-90">Upcoming</p>
                <h2 className="text-4xl font-bold mt-4">{upcomingInterviews}</h2>
              </div>
          ...
          {/* SCHEDULED */}
          <button onClick={() => {setSelectedType("SCHEDULED"); setSelectedInterview(null);}}
            ...
            <div className="flex justify-between items-start">
              <div>
                <p className="text-lg font-semibold opacity-90">Scheduled</p>
                <h2 className="text-4xl font-bold mt-4">{scheduledInterviews}</h2>
              </div>
          ...
          {/* COMPLETED */}
          <button onClick={() => {setSelectedType("COMPLETED"); setSelectedInterview(null);}}
            ...
            <div className="flex justify-between items-start">
              <div>
                <p className="text-lg font-semibold opacity-90">Completed</p>
                <h2 className="text-4xl font-bold mt-4">{completedInterviews}</h2>
              </div>
          ...
          {/* CANCELED */}
          <button
            onClick={() => {setSelectedType("CANCELED"); setSelectedInterview(null);}}
            ...
            <div className="flex justify-between items-start">
              <div>
                <p className="text-lg font-semibold opacity-90">Canceled</p>
                <h2 className="text-4xl font-bold mt-4">{canceledInterviews}</h2>
              </div>
        ...
        {selectedType && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredData.map((item) => (
              <Card key={item.interviewId} className="border-0 rounded-3xl shadow-elegant overflow-hidden hover:scale-[1.02] transition-all duration-300">
                <div className="bg-gradient-hero p-6">
                  <div className="flex justify-between items-start mb-5">
                    ...
                    <span
                      ...
                      ${ item.status === "COMPLETED" ? "bg-green-500" : item.status === "CANCELED" ? "bg-red-500" : "bg-blue-500"}`}>{item.status}
                    </span>
                  </div>
                  ...
                  <div className="space-y-4 text-muted-foreground">
                    <div className="flex items-center gap-3">
                      <CalendarDays className="h-5 w-5 text-primary" /> {new Date(item.scheduleDate).toLocaleDateString("en-GB")}
                    </div>                  
                    <div className="flex items-center gap-3">
                  ...
                  {/* BUTTON */}
                  <Button variant="hero" className="w-full mt-6 rounded-xl" 
                  onClick={() => navigate(`/models/interview/show-details-student/${item.interviewId}`)}>
                    View Full Details
                  </Button>
                </div>
              </Card>
