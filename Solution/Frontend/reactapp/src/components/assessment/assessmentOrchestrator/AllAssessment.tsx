...
const AllAssessment = () => {
  ...
  const handleRestore = async (id: number) => {
    try {
      const response = await fetch(`${WEBAPI_DOTNET_URL}/Assessment/restore/${id}`, { method: "POST" });

      if (response.ok) {
        const updatedData = assessments.map((item) =>item.id === id ? { ...item, isActive: true } : item);
        setAssessments(updatedData);
      } 
      else {
        ...
        {!showAssessments ? (
          <Card className="border-0 shadow-elegant overflow-hidden max-w-5xl mx-auto">
            <div className="bg-gradient-hero p-10 text-center">
              ...
              <Button variant="outline" size="lg" onClick={getAssessments}>
                Load Assessments
              </Button>
            </div>
          </Card>
                ...
                <div className="grid grid-cols-5 gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Button variant="outline" className="absolute right-3 top-2.5 text-gray-400 hover:text-gray-600">
                  ...
                  <div className="flex flex-col gap-2 justify-end">
                    <Button variant="outline" onClick={handleReset} className="px-6 py-2 border-2 border-gray-400 text-gray-700 rounded-lg font-semibold hover:bg-gray-100 transition">Reset
                    </Button>
                    <Button variant="outline" className="px-6 py-2 bg-[#E53935] text-white rounded-lg font-semibold hover:bg-[#D32F2F] transition">Filter</Button>
                  </div>
                </div>
              </CardContent>
            </Card>
            <div className="text-center mb-8">
              <Button variant="outline" className="bg-[#E53935] text-white px-6 py-2 rounded-lg font-semibold shadow-md hover:bg-[#D32F2F] transition"
                onClick={() => { handleReset(); setShowAssessments(true); }} > All Assessments 
              </Button>
            </div>
              ...
              <>
                        ...
                        <tbody>
                          {paginatedAssessments.map((assessment) => (
                            <tr key={assessment.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                              ...
                              <td className="px-6 py-5 text-center">
                                {assessment.isActive ?
                                  (<Button variant="outline" onClick={() => handleDelete(assessment.id)} className="bg-[#E53935] hover:bg-[#D32F2F] text-white px-4 py-2 rounded-lg font-semibold transition shadow-sm"> InActive</Button>) :
                                  (<Button variant="outline" onClick={() => handleRestore(assessment.id)} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded-lg font-semibold transition shadow-sm">Restore</Button>)}
                              </td>
                            </tr>
                          ))}
                        </tbody>
                ...
                <div className="flex items-center justify-between text-gray-600">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1}className="px-3 py-2 border rounded-xlborder-primary/20 rounded-lg text-gray-600 hover:bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">
                      Previous
                    </Button>
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                      <Button variant="outline" key={page} onClick={() => setCurrentPage(page)}
