...
import QuestionOptionsLocationState from "../entities/QuestionOptionsLocationState";
...
const QuestionOptions = () => {
  ...
  useEffect(() => {
    ...
    if (isCreateTestDraftPayload(fromNavigate)) {
      ...
      try {
          sessionStorage.setItem(CREATE_TEST_DRAFT_STORAGE_KEY,JSON.stringify(fromNavigate));
      } 
      catch {
  ...
  useEffect(() => {
    ...
    const roleId = Number( userData.roleid || userData.roleId || 0);
    ...
    fetch(`${WEBAPI_DOTNET_URL}/CreateTest/questions/${userId}`)
      ...
      .then((data) => { setQuestions(Array.isArray(data) ? data : []); setError(null);})
  ...
  const handleCreateTest = async () => {
    ...
    const storedTestData =testDraft ?? readCreateTestDraftFromSession() ??(isCreateTestDraftPayload(fromLocation) ? fromLocation : null);
    ...
    try {
        ...
        const userData = JSON.parse(sessionStorage.getItem("current") || "{}");
        const currentUserId = Number(userData.userid || userData.id || 0);
    ...
    <div className="w-full max-w-4xl mx-auto p-6">
      <Card className="border-0 shadow-soft ring-1 ring-slate-200">
        <CardHeader className="border-b border-slate-100 bg-slate-50/50">
          ...
          {testId && (<p className="text-sm text-slate-600">Test ID: {testId}</p>)}
          {testDraft && (<p className="text-sm text-slate-600">
        ...
        <CardContent className="p-8 space-y-8">
          ...
          {showActions && (
            <div className="pt-6 border-t flex gap-4">
              <Button className="flex-1 bg-red-600 text-white" onClick={() => { setShowInsertQuestion(true);setShowQuestions(false);}}>
                Add Questions
              </Button>
 
              <Button variant="outline" className="flex-1" onClick={() => { setShowQuestions(true); setShowInsertQuestion(false);}}>
                Select Questions
              </Button>
            </div>
          )}
            ...
            <div className="pt-6 space-y-4">
              ...
              {!loading && filteredQuestions.length > 0 ? (
                filteredQuestions.map((q) => (
                  <label key={q.questionId} className="flex gap-3 p-4 border rounded-xl">
                    <input type="checkbox" checked={selectedQuestions.includes(q.questionId)} onChange={() => toggleQuestion(q.questionId)}/>
                    <div>
              ...
              <div className="flex gap-4 pt-4">
                <Button className="bg-red-600 text-white flex-1" disabled={selectedQuestions.length === 0} onClick={handleCreateTest}>
                  Create Test
                </Button>
