    ...
    <div className="w-full max-w-8xl mx-auto p-6">
      <Card className="border-0 shadow-soft ring-1 ring-slate-200 griditem-4">
        <CardHeader className="border-b border-slate-100 bg-slate-50/50">
          <CardTitle className="text-2xl font-bold text-slate-900">SME Expertise Selection</CardTitle>
        </CardHeader>
        <CardContent className="p-8">
                  ...
                  {options[key as keyof typeof options].map((option) => (
                    <label
                      ...
                      className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer ${
                        form[key as keyof typeof form] === option
                    ...
                    >
                      <input
                        ...
                        name={key}
                        value={option}
                        checked={form[key as keyof typeof form] === option}
          ...
          <div className="mt-10 pt-6 border-t border-slate-100 flex gap-4">
            <Button className="flex-1 bg-red-600 hover:bg-red-700 text-white py-6 text-lg">
              Save Expertise
            </Button>
            <Button variant="outline" className="flex-1 py-6 text-lg" onClick={() => setForm({runtime: "", framework: "", layer: "", language: ""})}>
              Reset
            </Button>
          </div>
        </CardContent>
