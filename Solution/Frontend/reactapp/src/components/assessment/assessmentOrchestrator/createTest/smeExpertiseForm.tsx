...
const SMEExpertiseForm = () => {
  ...
  const fetchExpertiseOptions = async () => {
    try {
        const response = await fetch(`${WEBAPI_DOTNET_URL}/Expertise/options`);
        if (!response.ok) {
          throw new Error(
            "Failed to fetch expertise options"
          );
        }
        const data = await response.json();
        console.log("Expertise Options:",data);
        setOptions(data);
    } 
    catch (error) {
  ...
  const handleChange = (field: string, value: string) => {
      ...
      setForm(updatedForm);
      console.log("Updated SME expertise form:",updatedForm);
  };
      ...
      <Card className="border-0 shadow-soft ring-1 ring-slate-200 griditem-4">
        ...
        <CardContent className="p-8">
              ...
              <div  key={key} className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {options[key as keyof typeof options].map((option) => (
                    <label key={option} className={`flex items-center gap-3 p-3 rounded-xl border transition-all cursor-pointer ${
                        form[key as keyof typeof form] === option? "bg-red-50 border-red-200 ring-1 ring-red-200"
                        : "bg-white border-slate-200 hover:border-red-200 hover:bg-slate-50"}`} >
                      <input type="radio" name={key} value={option} checked={ form[key as keyof typeof form] === option}
                          className="h-4 w-4 accent-red-600"  onChange={(e) => handleChange( key, e.target.value)}/>
          ...
          <div className="mt-10 pt-6 border-t border-slate-100 flex gap-4">
            <Button className="flex-1 bg-red-600 hover:bg-red-700 text-white py-6 text-lg" onClick={handleSave} >
              Save Expertise
            </Button>
