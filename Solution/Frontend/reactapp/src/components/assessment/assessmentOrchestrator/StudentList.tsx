...
import {ArrowLeft, Search, Users,Loader2, Eye,UserX,Inbox,} from "lucide-react";
...
function StatusBadge({ status }: { status: StudentStatus }) {
  ...
  return (<Badge className={`font-medium ${styles[status]}`} variant="secondary"> {status}</Badge> );
}
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
           ...
           <Button variant="outline" className="gap-2" onClick={() => navigate("/models/membership/dashboard")}>
            <ArrowLeft className="h-4 w-4" /> Back to Dashboard  </Button>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Card className="md:col-span-2">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Search Student</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <TableBody>
                  {filteredStudents.map((student) => (
                    <TableRow key={student.id} className="transition-colors hover:bg-muted/50" >
                      ...
                      <TableCell className="text-muted-foreground">{maskContact(student.contact)} </TableCell>
                      ...
                      <TableCell className="text-right">
                        {student.status === "BLOCKED" ? (
                          <Button size="sm" variant="ghost" disabled className="gap-2 text-muted-foreground">
                            <UserX className="h-4 w-4" /> Blocked </Button>
                        ) : (
                          <Button size="sm" variant="ghost" className="gap-2" onClick={() => navigate(`/models/membership/UserProfile/${student.id}`)}>
