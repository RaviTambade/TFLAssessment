...
const CompletedAssessmentDetails = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${WEBAPI_DOTNET_URL}/Score/GetAssessmentAnswerDetails/${user.userid}/${assessmentId}` )
      .then((res) => {
        if (!res.ok) {
          throw new Error("No answer details found");
        }
        return res.json();
      }).then((data) => setAnswers(data || [])).catch(() => setAnswers([])).finally(() => setLoading(false));
  }, [assessmentId]);
  ...
  return (
    <section className="py-16 bg-background min-h-screen">
      <div className="container mx-auto px-4">
        ...
        <button onClick={() => navigate(-1)} className="mb-6 flex items-center gap-2 text-primary font-medium hover:underline" >
          <ArrowLeft size={18} />
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            {answers.map((item, index) => (
              <Card key={item.questionId} className="border shadow-md hover:shadow-lg transition-all duration-300" >
                <CardContent className="p-6">
                  ...
                  <div className="grid gap-3">
                    {[
                      ...
                      const isCorrectOption = option.key === item.correctAnswer;

                      const isSelectedOption = option.key === item.selectedOption;

                      let className ="flex items-center justify-between rounded-lg border p-4 transition-all duration-200";
                      ...
                      return (
                        <div key={`${item.questionId}-${option.key}`} className={className}>
