...
import AssignmentSummary from "./entities/AssignmentSummary";
...
export default function AssignAssessment() {
  ...
  const getAssessmentIdsFromResponse = (data: unknown) => {
    const singleId = getNumberValue(data, ["assessmentId", "AssessmentId", "id", "Id", "assignedAssessmentId", "AssignedAssessmentId"]);
    if (singleId) {
      return [singleId];
    }
    if (!data || typeof data !== "object") {
  ...
  const filteredTests = tests.filter(t => (t?.title ?? "").toLowerCase().includes(searchTest.toLowerCase()));
  const handleAssign = async () => { setSuccessMessage(null); setErrorMessage(null);
    ...
    try {
      ...
      if (status === "Cancelled") {
        const res = await fetch(`${WEBAPI_DOTNET_URL}/Assessment/cancel/test/${selectedTest}`,
          {
            method: "POST"
          }
        );
        if (!res.ok) {
          ...
          const message = getResponseMessage(errorData, "Failed to cancel assessment");
          throw new Error(message);
        }
        const result = await readResponse(res);
        const cancelledCount = Number(result && typeof result === "object" && "cancelledCount" in result ? (result as { cancelledCount?: number }).cancelledCount ?? 0 : 0);
        ...
        setSuccessMessage(
          getResponseMessage(result, `Assessment cancelled successfully. ${cancelledCount} record(s) updated.`)
        );
        return;
      }
      const dto = {StudentIds: selectedStudents, TestId: selectedTest, AssignedBy: assignedBy, ScheduledAt: new Date(`${date}T${time}:00`).toISOString(), Status: status};
      const res = await fetch(`${WEBAPI_DOTNET_URL}/Assessment/assigned`, { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(dto)});
      if (!res.ok) {
        const errorData = await readResponse(res);
        const message = getResponseMessage( errorData, "Failed to assign assessment");
        throw new Error(message);
      }
      const result = await readResponse(res);
      if (
      ...
      const assessmentIds = responseAssessmentIds.length > 0 ? responseAssessmentIds : await fetchLatestAssessmentIds(testName, selectedStudents, dto.Status, date);
      setSummary({ AssessmentIds: assessmentIds, TestName: testName, Status: dto.Status, ScheduledAt: dto.ScheduledAt, StudentNames: studentNames});
      setSuccessMessage(
        getResponseMessage(
          result,
      ...
      <div className="w-full max-w-4xl">
        ...
        {successMessage && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
            <div className="w-full max-w-4xl rounded-lg border border-green-300 bg-green-50 p-8 text-green-800 shadow-2xl">
              ...
              <button type="button" onClick={() => { setSuccessMessage(null); setSummary(null); reset(); }}
        ...
        {errorMessage && (
          <div className="mb-4 flex items-center justify-between rounded-lg border border-red-300 bg-red-100 px-4 py-3 text-red-800 shadow-sm">
            ...
            <button type="button" onClick={() => setErrorMessage(null)} className="rounded-md bg-red-600 px-3 py-1 text-sm font-medium text-white transition hover:bg-red-700">
              OK
            </button>
          </div>
        )}
        <div className="bg-white p-8 rounded-xl shadow-lg border border-orange-200 space-y-6">
          <section>
            ...
            <input
              placeholder="Search tests..." className="border p-2 w-full mb-3 rounded-md focus:ring-2 focus:ring-orange-500 outline-none" value={searchTest}
              onChange={(e) =>
            ...
            <div className="max-h-60 overflow-y-auto border rounded-md p-2">
              {filteredTests.map(t => (
                ...
                >
                  <input type="radio" name="test" className="accent-orange-600" checked={selectedTest === t.id} onChange={() => setSelectedTest(t.id)}/>
                  <div>
              ...
              >
                {selectedStudents.map(id => (
                  <span key={id} className="bg-orange-100 text-orange-700 px-2 py-1 rounded text-xs font-bold flex items-center gap-1">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); toggleStudent(id);}} className="hover:text-orange-900 ml-1">
                      ×
                    </button>
                  </span>
                ))}
                <input
                  type="text"
                  placeholder={selectedStudents.length === 0 ? (status === "Cancelled" ? "Students are not required for cancellation" : "Search and select students...") : ""}
                  value={searchStudent}
                  onChange={(e) =>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <section>
              ...
              <div className="flex gap-2">
                <input type="date" className="border p-2 flex-1 rounded-md outline-none focus:ring-2 focus:ring-orange-500" value={date} onChange={(e) => setDate(e.target.value) }/>
                <input type="time" className="border p-2 flex-1 rounded-md outline-none focus:ring-2 focus:ring-orange-500" value={time} onChange={(e) => setTime(e.target.value) }/>
              </div>
            </section>
          </div>
          <div className="pt-6 border-t flex gap-4">
            ...
            <button onClick={reset} className="flex-1 border-2 border-gray-300 text-gray-600 py-3 rounded-lg font-bold hover:bg-gray-50 transition">
              Reset
            </button>
          </div>
        </div>
