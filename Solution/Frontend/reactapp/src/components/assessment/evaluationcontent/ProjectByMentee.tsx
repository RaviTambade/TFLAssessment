    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-start justify-between">
          <div>
            <h1 className="text-4xl font-bold bg-gradient-primary bg-clip-text text-transparent"> Student Projects</h1>
            <p className="text-muted-foreground mt-2"> Monitor and review all mentee projects from one place.</p>
          </div>

          <Button variant="outline" onClick={() => navigate("/models/membership/dashboard")} className="flex items-center gap-2">
        ...
        {loading && (<div className="text-center py-10 text-lg">Loading projects... </div>)}
        {error && (<div className="text-center text-red-600 font-medium mb-6"> {error} </div> )}
        {/* Project Cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {!loading && projects.length === 0 && (<div className="col-span-full text-center text-gray-500"> No projects available.</div>)}
           {projects.map((project: any) => (
            <Card key={project.projectId} className="hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border-l-4 border-red-500">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
