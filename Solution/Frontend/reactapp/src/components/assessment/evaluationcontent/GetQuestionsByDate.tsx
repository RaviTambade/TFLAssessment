...
const GetQuestionsByDate: React.FC = () => {
    ...
    const fetchQuestions = async () => {
        try {
            const response = await fetch(`http://localhost:8080/api/questions/recent?fromDate=${fromDate}&toDate=${toDate}`);
            if (!response.ok) {
                        ...
                        <table className="w-full">
                            ...
                            <tbody>
                                {questions.length > 0 ? (questions.map((question) => (
                                        <tr key={question.questionId} className="border-b border-border hover:bg-muted/40 transition-all">
                                            <td className="px-6 py-4">{question.questionId}</td>
                                            <td className="px-6 py-4">{question.description}</td>
                                            <td className="px-6 py-4">{question.questionType}</td>
